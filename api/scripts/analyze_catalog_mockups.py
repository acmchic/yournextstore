    ...
    try:
        ...
        async with database.transaction() as cursor:
            for catalog in catalogs:
                ...
                kits = {str(item.get("locationCode", "")).lower(): item for item in guideline.get("design_kits", [])}
                assets = await database.fetch_all(
                    "select id, placement, width, height from catalog_assets where catalog_id=%s and status='active' order by id",
                    (catalog["id"],),
                )
                for placement in ("front", "back"):
                    asset = next((item for item in assets if item["placement"] == placement), None)
                    asset = asset or next((item for item in assets if item["placement"] == "front"), None)
                    ...
                    area = {
                        ...
                        "height": percent(kit.get("designAreaHeight"), fallback["height"] * 100) / 100,
                    }
                    status = "ready" if placement in kits else "needs_review"
                    await cursor.execute(
                        ...
                        """,
                        (catalog["id"], placement, asset["id"], json.dumps(area), asset["width"], asset["height"], "v1", status),
                    )
    finally:
