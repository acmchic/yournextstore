	...
	return (
		<>
			...
			<div className="space-y-6 p-6">
				<div>
					<h1 className="text-2xl font-semibold">{catalog ? "Edit catalog" : "New catalog"}</h1>
				</div>
				<form onSubmit={submit} className="grid max-w-3xl gap-4 rounded-xl border p-6 sm:grid-cols-2">
					<Input
						placeholder="Name"
					...
					<Input
						...
						onChange={(e) => form.setData("material", e.target.value)}
					/>
					<Input
						type="number"
					...
					/>
					<textarea
						className="min-h-28 rounded-md border bg-background p-3 sm:col-span-2"
						placeholder="Description override"
						value={form.data.description_override}
						onChange={(e) => form.setData("description_override", e.target.value)}
					/>
								...
								<tbody>
									{variants.map((v: any) => (
										<tr className="border-t" key={v.id}>
											...
											<td className="p-3">
												{(v.default_price_minor / 100).toFixed(2)} {v.currency}
											</td>
											<td className="p-3">
												{v.stock_quantity} ({v.stock_policy})
											</td>
										</tr>
									))}
								</tbody>
