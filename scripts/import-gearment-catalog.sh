...
if command -v docker >/dev/null 2>&1 || command -v docker-compose >/dev/null 2>&1; then
  ...
  for migration in 001_schema.sql 002_seed_catalog.sql 003_gearment_catalog.sql; do
  ...
  "${compose[@]}" run --rm --no-deps \
    ...
    --manifest /app/catalog-import.json --apply --refresh-assets
else
  env_value() {
  ...
  (
    ...
    PYTHONPATH=. .venv/bin/python -m app.cli sync-gearment-catalog \
      --manifest catalog-import.json --apply --refresh-assets
  )
fi
